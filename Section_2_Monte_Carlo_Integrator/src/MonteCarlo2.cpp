...
std::function<bool(double,double,double)> functionexample)
{
    int count = 0;
    ...
    for(int i = 0; i < n_points; i++)
    {
        ...
        if(functionexample(x,y,z)) count++;
    }
...
int main()
{
    ...
    double UnitSphereVol = IntegrateMonteCarlo3D(N_points, -1.0, 1.0, &InsideUnitSphere); // & for the function too apparently
